...
router.post('/join', authenticate, async (req, res) => {
  ...
  try {
    ...
    const isRegistered = tournament.participants.some(p => p.userId === req.user.id);
    if (!isRegistered) return res.status(403).json({ message: 'You must register for the tournament first' });
